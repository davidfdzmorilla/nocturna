...
async def test_smoke_llamada_real_autorizada_ejecutada_y_contabilizada_en_budget_guard(
    ...
    # Acota el lado de entrada: un prompt de una línea no debería inflar
    # `tokens_in` salvo que `setting_sources=[]` no esté evitando cargar
    # contexto de proyecto (ver `build_options`).
    assert result.tokens_in < 1_500, (
        f"tokens_in={result.tokens_in} es alto para un prompt de una línea; "
        "puede indicar que se está cargando contexto de proyecto pese a "
        "setting_sources=[]"
    )
    assert result.total_tokens < 5_000, (
        "se esperaba que la llamada de este test quedara por debajo de 5 000 "
        f"tokens (ver docstring del módulo); gastó {result.total_tokens}"
    )

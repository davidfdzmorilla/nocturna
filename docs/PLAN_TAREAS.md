...
- **Pendiente**: verificación V1 (envoltorio bajo entorno de `launchd` con `--dry-run`) y V3 (ejecución fuera de ventana: se espera código 8, `Run` en `KILLED`, cero tokens), instalación del agente y `sudo pmset repeat wakeorpoweron` con el portátil enchufado. Todo eso lo ejecuta el autor a mano.
